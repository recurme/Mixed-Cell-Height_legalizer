...
template <typename T>
void WriteMap(const std::string& fileNamePrefix, const std::vector<std::vector<T>>& map) {
#ifndef WRITE
    return;
#endif
...
template <typename T>
void DrawMap(const std::string& fileNamePrefix, const std::vector<std::vector<T>>& map, double xscale = 1.0, double yscale = 1.0) {
// #ifndef DRAW
//     return;
// #endif
...
template <typename T>
void DrawMap2(const std::string& fileNamePrefix,
...
template <typename T>
void PrintMapStat(const std::string& name, const std::vector<std::vector<T>>& map) {
...
struct sorting_order {
    static bool desc(const std::pair<key, value>& left, const std::pair<key, value>& right) { return left.first > right.first; }
    static bool asc(const std::pair<key, value>& left, const std::pair<key, value>& right) { return left.first < right.first; }
};