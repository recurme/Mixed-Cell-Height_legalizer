...
#define range(x, lo, hi) (std::max((lo), std::min((hi), (x))))
...
void DrawCanvas::SetLineThickness(double size) {
    ...
    if (size == 0.0) {
        ...
        gdImageSetThickness(_image, (int)std::max(1.0, size));
    }
}
...
void DrawCanvas::DrawString(std::string str, double x, double y) {
...
void DrawCanvas::Save(std::string file) {
    FILE *fp;
    if (_image == NULL) {
        return;
    }
