...
public:
    inline void set() {
        up_.set(std::numeric_limits<T>::lowest(), std::numeric_limits<T>::lowest()),
            lo_.set(std::numeric_limits<T>::max(), std::numeric_limits<T>::max());
    }
    inline void set(const T& x, const T& y) {
        up_.set(x, y);
        lo_.set(x, y);
...
template <typename T>
void GetMedianTwo(std::vector<T>& vals, T& l, T& h) {
...
// Note 2: Key can be std::pair<ScoreT1, ScoreT2>
template <typename Val, typename Key, typename Compare>
void ComputeAndSort(std::vector<Val>& values, std::function<Key(Val)> score, Compare comp, bool stable = false) {
    std::vector<std::pair<Key, Val>> kvs;
    kvs.reserve(values.size());
    for (auto& val : values) kvs.push_back({score(val), val});
    auto loc_comp = [&](const std::pair<Key, Val>& a, const std::pair<Key, Val>& b) { return comp(a.first, b.first); };
    if (stable)
...
template <class T>
void GetWinElem(std::vector<T>& candidates,
